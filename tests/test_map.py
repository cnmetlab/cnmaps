...
def test_map_load():
    ...
    assert (beijing['省/直辖市'] == '北京市' and
            beijing['市'] is None and
            beijing['区/县'] is None and
            beijing['级别'] == '省')

    beijing = get_adm_maps(city='北京市')[0]
    assert (beijing['市'] == '北京市' and
            beijing['区/县'] is None and
            beijing['级别'] == '市')
    ...
    assert (beijing_chaoyang['区/县'] == '朝阳区' and
            beijing_chaoyang['级别'] == '区县')
...
def test_map_operator():
    ...
    # 加法操作符(并集)
    assert isinstance(
        get_adm_maps(province='四川省')[0]['geometry'] + get_adm_maps(province='重庆市')[0]['geometry'] + get_adm_maps(province='贵州省')[0]['geometry'], MapPolygon)
    assert isinstance(
    ...
    assert isinstance(
        get_adm_maps(level='国')[0]['geometry'] - get_adm_maps(province='山西省')[0]['geometry'] + get_adm_maps(province='山西省')[0]['geometry'], MapPolygon)
    assert round((get_adm_maps(level='国')[
                 0]['geometry'] - get_adm_maps(province='山西省')[0]['geometry']).area, 2) == 949.32
...
def test_only_polygon_and_record():
    ...
    polygon = get_adm_maps(city='北京市', record='first',
                           level='区县', only_polygon=True)
